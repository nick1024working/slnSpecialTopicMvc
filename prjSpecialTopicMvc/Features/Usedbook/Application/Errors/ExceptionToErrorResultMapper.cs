...
public static class ExceptionToErrorResultMapper<T>
{
    ...
    public static Result<T> Map(Exception ex, ILogger logger, string? context = null)
    {
        var source = typeof(T).Name;
        var location = context ?? new System.Diagnostics.StackTrace().GetFrame(1)?.GetMethod()?.Name ?? "Unknown";

        logger.LogError(ex,
            "[{Source}.{Location}] 發生例外：{ExceptionType} - {Message}",
            source,
            location,
            ex.GetType().Name,
            ex.Message
